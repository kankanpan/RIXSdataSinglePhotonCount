...
def saveCSV(title, data):
    eel.spawn(saveCSVWindow(title, data))

def saveCSVWindow(title, data):
    file_name = startTkSaveWindow()
    result = {}
...
def saveIgor(title, data):
    eel.spawn(saveIgorWindow(title, data))

def saveIgorWindow(title, data):
    file_name = startTkSaveWindow()
    result = []
    for i in range(len(title)):
        result.append(data[i], name=title[i])
...
@eel.expose
def clipImage(current_id, current_index, range):
    data, max = db.getImageByLayer(current_id, current_index)
    dset = cv2.imdecode(data, cv2.IMREAD_GRAYSCALE)
